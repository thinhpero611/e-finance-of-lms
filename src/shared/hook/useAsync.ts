...
export function useSingleAsync<T>(asyncFunction, options?: IOptions) {
	...
	console.log('before execute method', state.status)
	const execute = useCallback<(args:any)=>Promise<T>>(
		(...args) => {
			setState((prevState) => ({
				...prevState,
				status: 'loading',
...
export const useAsync = function (...params: Array<Function | [Function, IOptions?]>) {
	return params
		.map((item) => {
